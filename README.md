...
Our design kits are available in Figma, Sketch, and Adobe XD formats. Of these formats, the USWDS design kit for **Figma** is the most up-to-date. Unfortunately, Adobe XD is no longer supported by Adobe, so we won't be making further updates to this file — it's frozen as-is :ice_cube:.
...
The `sketch/libraries/uswds` library includes the following:
...
:warning: **Don't directly edit the core USWDS library file.** All USWDS system and theme libraries are designed to keep your local files in sync with most current USWDS design tokens. Don't customize the library files — instead, use the styles and symbols in the `uswds-project` file to customize your project.
...
:warning: **If you leave the project file in the `uswds-for-designers` folder, it'll be overwritten if you ever grab updated files from GitHub, and that's truly no fun.**
...
**4.** For each color you want to change, first swap the color symbol in the artboard with the replacement color from the `uswds/colors/system` library. Here we're swapping the `primary` default of `blue 60v` with `green-60v`.
...
## :open_book: History / changelog

### :coffee: January 2025: USWDS design kit for Figma beta 0.3 (USWDS 3.11.0)

In November 2024, we launched the USWDS design kit for Figma as a beta on the Figma Community site. In January 2025, we added a direct download in this repo, available as a `.fig` file within the `figma` folder. It can be simpler, if possible, to [get the design kit directly from Figma](https://www.figma.com/community/file/1440921849343185329/uswds-design-kit-beta), but unfortunately there's no Community equivalent in FigmaGov.

The USWDS design kit for Figma contains 42 USWDS components built with USWDS design tokens, using variables and smart layouts. Report bugs or request features [at this repo](https://github.com/uswds/uswds-for-designers/issues), or email uswds@gsa.gov.

Documentation and complete changelog included in the kit itself.
